(function() {
  ...
  var DEFAULT_ROW_HEIGHT = '24px';
  var DEFAULT_COLUMN_WIDTH = '64px';
  var SMALL_COLUMN_WIDTH = '42px';
  var BIG_COLUMN_WIDTH = '80px';
  var HEADER_HEIGHT = '54px';
  var DEFAULT_TIMEOUT = 2000;
  var ALARM_COLOR = '#ff0000';
  ...
  var handle_scrolls = function(i_scrolls, i_id, i_textarea, i_restore) {
    var scrolls = i_textarea.hmi_handleScrollParams(i_scrolls[i_id], i_restore);
    if (scrolls.viewport_left > 0 || scrolls.viewport_top) {
      i_scrolls[i_id] = scrolls;
    }
    else {
      delete i_scrolls[i_id];
    }
  };

  var update_scrolls = function(i_scrolls, i_params) {
    // console.log(JSON.stringify(i_params, undefined, 2));
    switch (i_params.action) {
    case ContentManager.COPY:
      var objs = i_params.objects, i, l = i_scrolls.length, scrolls, src, data, attr, copy;
      for (src in objs) {
    ...
    case ContentManager.MOVE:
      var objs = i_params.objects, i, l = i_scrolls.length, scrolls, src;
      for (src in objs) {
        if (objs.hasOwnProperty(src)) {
          for (i = 0; i < l; i++) {
            scrolls = i_scrolls[i];
    ...
    case ContentManager.DELETE:
      var objs = i_params.objects, i, l = i_scrolls.length, scrolls, src = i_params.source;
      if (objs) {
        for (src in objs) {
          if (objs.hasOwnProperty(src)) {
            for (i = 0; i < l; i++) {
              delete i_scrolls[i][src];
            }
          }
        }
      }
  ...
  var get_handler = function(i_desc, i_lab, i_htm, i_txt, i_jso) {
    if (!i_desc) {
      return false;
    }
    else if (i_desc.jsonfx) {
      return {
          cont : i_jso,
          desc : i_desc
      };
    }
    else if (!i_desc.multilingual) {
      return {
          cont : i_txt,
          desc : i_desc
      };
    }
    else if (i_desc.multiedit) {
      return {
          cont : i_lab,
          desc : i_desc
      };
  ...
  var get_htm_editor = function(i_hmi, i_adapter) {
    ...
    var reload = function(i_data, i_language, i_success, i_error) {
      info_lang.hmi_text('language: "' + i_language + '"');
      if (textarea.file) {
        handle_scrolls(scrolls, textarea.file, textarea, false);
      ...
      if (i_data && i_data.file) {
        cms.getObject(i_data.file, i_language, ContentManager.RAW, function(i_raw) {
          ...
          if (i_raw !== undefined) {
            ...
            handle_scrolls(scrolls, textarea.file, textarea, true);
          }
          i_success();
        }, function(i_exception) {
          textarea.hmi_html('');
  ...
  var get_txt_preview = function(i_hmi, i_adapter) {
    ...
    var reload = function(i_data, i_language, i_success, i_error) {
      if (textarea.file_raw) {
        handle_scrolls(scrolls_raw, textarea.file_raw, textarea, false);
        delete textarea.file_raw;
      }
      if (textarea.file_build) {
        handle_scrolls(scrolls_build, textarea.file_build, textarea, false);
        ...
        case ContentManager.RAW:
          cms.getObject(i_data.file, i_language, ContentManager.RAW, function(i_raw) {
            ...
            if (i_raw !== undefined) {
              ...
              handle_scrolls(scrolls_raw, textarea.file_raw, textarea, true);
            }
            i_success();
          }, function(i_exception) {
            textarea.hmi_value('');
        ...
        case ContentManager.BUILD:
          cms.getObject(i_data.file, i_language, ContentManager.BUILD, function(i_build) {
            ...
            if (i_build !== undefined) {
              ...
              handle_scrolls(scrolls_build, textarea.file_build, textarea, true);
            }
            i_success();
          }, function(i_exception) {
            textarea.hmi_value('');
  ...
  var get_preview = function(i_hmi, i_adapter) {
    ...
    cms.getDescriptors(function(i_ext, i_desc) {
      handlers[i_ext] = get_handler(i_desc, lab, htm, txt, jso);
    });
    var container = {
        type : 'container',
  ...
  var get_content_editor = function(i_hmi) {
    ...
    var refactoring_adapter = {
        ...
        updateScrollParams : function(i_params) {
          update_scrolls(scroll_positions, i_params);
        },
  ...
  window.getContentEditor = get_content_editor;
}());
